...
└── {YYYY-MM-DD}_v{N}_lgpd-report.json
```
...
2. Create output directory with versioning:
   ...
   if [ -f "$ENSURE_SCRIPT" ]; then
       OUTPUT_PATH=$(bash "$ENSURE_SCRIPT" "{project-name}" --v "lgpd-report.json")
   else
       mkdir -p "/mnt/d/Opencode/{project-name}"
       OUTPUT_PATH="/mnt/d/Opencode/{project-name}/lgpd-report.json"
   fi
   ```
